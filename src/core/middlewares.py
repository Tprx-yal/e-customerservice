...
class HttpAuditLogMiddleware(BaseHTTPMiddleware):
    ...
    async def after_request(self, request, response, process_time):
        # 智能生成摘要
        ...
        if not summary:
            ...
            if hasattr(request.state, 'error'):
                summary = f"错误: {request.state.error}"
            elif response.status_code >= 400:
                summary = f"请求失败 - {response.status_code}"
            else:
                summary = f"成功处理 {request.method} 请求到 {request.url.path}"
        ...
        data = {
            ...
            "summary": summary[:255],  # 限制长度防止数据库字段溢出
        }
