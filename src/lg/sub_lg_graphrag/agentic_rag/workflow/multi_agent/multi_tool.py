...
# 导入guardrails逻辑
from ...components.guardrails.node import create_guardrails_node
# 导入分解节点
from ...components.planner import create_planner_node
# 导入工具选择节点
from ...components.tool_selection import create_tool_selection_node
# 导入 text2cypher 节点
from ...components.cypher_tools import create_cypher_query_node
# 导入Cypher示例检索器基类
...
# 导入自定义工具函数节点
from ...components.customer_tools import create_graphrag_query_node
...
class AgentState(InputState):
    ...
    question: str # 这个参数用来与子图进行交互
    answer: str # 这个参数用来与子图进行交互
...
def create_multi_tool_workflow(
    ...
    main_graph_builder.add_node(guardrails) # type: ignore
