...
from lg.sub_lg_graphrag.agentic_rag.constants import NO_CYPHER_RESULTS
from ...components.state import PredefinedCypherInputState
...
from ...components.predefined_cypher.descriptions import QUERY_DESCRIPTIONS
...
def create_predefined_cypher_node(
    ...
    async def predefined_cypher(
        ...
        statement = predefined_cypher_dict.get(params.get("query")) # type: ignore
        print(f"Executing predefined Cypher statement: {statement} with params: {params}")
        if statement is not None:
            records = graph.query(query=statement, params=params.get("parameters")) # type: ignore
