...
from .prompts import create_text2cypher_generation_prompt_template, create_text2cypher_validation_prompt_template, create_text2cypher_correction_prompt_template
...
def validate_cypher_query_with_schema(
    ...
    """
    from ...components.text2cypher.validation.models import (
...
def create_text2cypher_generation_node(
    ...
    async def generate_cypher(state: CypherInputState) -> Dict[str, Any]:
        ...
        return generated_cypher # type: ignore

    return generate_cypher # type: ignore
    ...
    async def validate_cypher(state: CypherState) -> Dict[str, Any]:
        ...
        if llm is not None and llm_validation:
            llm_errors = await validate_cypher_query_with_llm(
                validate_cypher_chain=validate_cypher_chain, # type: ignore
                question=state.get("task", ""), # type: ignore
                graph=graph,
                cypher_statement=cypher_statement,
            )
            print("llm_errors:", llm_errors)
        ...
        if errors:  # 真正的语法错误
            correct_cypher_chain = correction_cypher_prompt | llm | StrOutputParser() # type: ignore
            ...
            "cyphers": [
                CypherOutputState(
                    **{
                        ...
                        "parameters": None,
                        "errors": cypher["errors"], # type: ignore
