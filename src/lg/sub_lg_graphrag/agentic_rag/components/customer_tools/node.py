...
from graphrag.storage.file_pipeline_storage import FilePipelineStorage # type: ignore
...
class GraphRAGAPI:
    ...
    async def initialize(self):
        ...
        self.storage = FilePipelineStorage(base_dir=str(output_dir))
...
def create_graphrag_query_node(
    ...
    async def graphrag_query(
            state: Dict[str, Any],
    ) -> Dict[str, List[GraphRAGQueryOutputState] | List[str]]: # type: ignore
        """
        执行GraphRAG查询并返回结果。
        """
        errors = list()
    ...
    return graphrag_query # type: ignore
