...
from lg.sub_lg_graphrag.agentic_rag.workflow.multi_agent import create_multi_tool_workflow
...
# 构建日志记录器
logger = logging_config.setup_logger(service="lg_builder")
...
async def analyze_and_route_query(
    ...
    if settings.AGENT_SERVICE == ServiceType.DEEPSEEK:
        model = ChatOpenAI(base_url=settings.DEEPSEEK_BASE_URL, api_key=settings.DEEPSEEK_API_KEY, # type: ignore
                           model=settings.DEEPSEEK_MODEL, temperature=0.7, tags=["router"])
...
def route_query(
    ...
    "respond_to_general_query", "get_additional_info", "create_research_plan", "create_image_query", "create_file_query", "breake2human"]:
    """根据查询分类确定下一步操作。
...
async def respond_to_general_query(
    ...
    if settings.AGENT_SERVICE == ServiceType.DEEPSEEK:
        model = ChatOpenAI(base_url=settings.DEEPSEEK_BASE_URL, api_key=settings.DEEPSEEK_API_KEY, # type: ignore
                           model=settings.DEEPSEEK_MODEL, temperature=0.7, tags=["general_query"])
    else:
        model = ChatOllama(model=settings.OLLAMA_AGENT_MODEL, base_url=settings.OLLAMA_BASE_URL, temperature=0.7,
                           tags=["general_query"])
...
async def get_additional_info(
    ...
    if settings.AGENT_SERVICE == ServiceType.DEEPSEEK:
        model = ChatOpenAI(base_url=settings.DEEPSEEK_BASE_URL, api_key=settings.DEEPSEEK_API_KEY, # type: ignore
                           model=settings.DEEPSEEK_MODEL, temperature=0.7, tags=["additional_info"])
    else:
        model = ChatOllama(model=settings.OLLAMA_AGENT_MODEL, base_url=settings.OLLAMA_BASE_URL, temperature=0.7,
    ...
    try:
        ...
        with Image.open(image_path) as img:
            ...
            if width <= max_size and height <= max_size:
                ...
                resized_img = img.resize((new_width, new_height), resample=Image.LANCZOS) # type: ignore
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    if settings.AGENT_SERVICE == ServiceType.DEEPSEEK:
                        model = ChatOpenAI(base_url=settings.DEEPSEEK_BASE_URL, api_key=settings.DEEPSEEK_API_KEY, # type: ignore
                                           model=settings.DEEPSEEK_MODEL, temperature=0.7, tags=["image_query"])
                    else:
...
async def create_file_query(
        state: AgentState, *, config: RunnableConfig
) -> Dict[str, List[BaseMessage]]: # type: ignore
    """Create a file query."""
...
async def create_research_plan(
    ...
    if settings.AGENT_SERVICE == ServiceType.DEEPSEEK:
        model = ChatOpenAI(base_url=settings.DEEPSEEK_BASE_URL, api_key=settings.DEEPSEEK_API_KEY, # type: ignore
                           model=settings.DEEPSEEK_MODEL, temperature=0.7, tags=["research_plan"])
    else:
        model = ChatOllama(model=settings.OLLAMA_AGENT_MODEL, base_url=settings.OLLAMA_BASE_URL, temperature=0.7,
                           tags=["research_plan"])
...
async def check_hallucinations(
    ...
    if settings.AGENT_SERVICE == ServiceType.DEEPSEEK:
        model = ChatOpenAI(base_url=settings.DEEPSEEK_BASE_URL, api_key=settings.DEEPSEEK_API_KEY, # type: ignore
                           model=settings.DEEPSEEK_MODEL, temperature=0.7, tags=["hallucinations"])
    else:
        model = ChatOllama(model=settings.OLLAMA_AGENT_MODEL, base_url=settings.OLLAMA_BASE_URL, temperature=0.7,
                           tags=["hallucinations"])
...
# 添加节点
builder.add_node(analyze_and_route_query) # type: ignore
builder.add_node(respond_to_general_query) # type: ignore
builder.add_node(get_additional_info) # type: ignore
builder.add_node("create_research_plan", create_research_plan)  # type: ignore # 这里是子图
builder.add_node(create_image_query) # type: ignore
builder.add_node(create_file_query) # type: ignore
