...
# GraphRAG项目根目录路径
PROJECT_DIR = r"D:\python_code\CustomerService\customerservice\graphrag_local"
...
# 输出目录
OUTPUT_DIR = r"D:\python_code\Simple_test\graphrag-2.7.0\data\pdf_prompt_turn_output"
...
async def run_prompt_tune(
    ...
    try:
        ...
        try:
            ...
            logger.error(f"generate_indexing_prompts 验证错误详情: {str(e)}")
            logger.error(f"config 类型: {type(graphrag_config)}")
            logger.error(f"selection_method 类型: {type(doc_selection)}")
            raise
