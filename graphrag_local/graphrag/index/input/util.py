...
async def generate_image_descriptions(
    ...
    if not image_dir.exists() or not image_dir.is_dir():
        logger.warning(f"图片目录不存在或不是目录: {image_dir}")
        return {}
    ...
    if not image_files:
        logger.warning(f"图片目录中没有找到图片: {image_dir}")
        return {}

    logger.info(f"找到 {len(image_files)} 张图片，开始生成描述")
                ...
                try:
                    async with session.post(
                        ...
                        if response.status == 200:
                            ...
                            logger.info(f"成功生成图片描述: {image_path.name}")
                            return str(image_path), description
                        else:
                            error_text = await response.text()
                            logger.error(
                                f"API请求失败 (尝试 {attempt + 1}/{max_retries}): {response.status} - {error_text}")
                            if attempt < max_retries - 1:
                                await asyncio.sleep(retry_delay * (attempt + 1))  # 指数退避
                            else:
                                return str(image_path), f"[描述生成失败: API错误 {response.status}]"
                except Exception as e:
                    logger.error(f"处理图片时出错 (尝试 {attempt + 1}/{max_retries}): {str(e)}")
                    if attempt < max_retries - 1:
    ...
    for i, batch in enumerate(batches):
        logger.info(f"处理批次 {i + 1}/{len(batches)}, 包含 {len(batch)} 张图片")
        ...
        if output_file is not None:
            ...
            logger.info(f"已将当前描述保存到: {output_file}")

        # 批次之间添加延迟，避免API限制
        if i < len(batches) - 1:
            time.sleep(1)

    logger.info(f"所有图片处理完成，共生成 {len(descriptions)} 个描述")

    # 最终保存
    if output_file is not None and descriptions:
        with open(output_file, 'w', encoding='utf-8') as f:
            json.dump(descriptions, f, ensure_ascii=False, indent=2)
        logger.info(f"所有描述已保存到: {output_file}")
...
def generate_image_descriptions_sync(
    ...
    base_url = config.image_description_base_url if hasattr(config,
                                                            "image_description_base_url") else "https://api.openai-proxy.org/v1"

    logger.info(f"使用模型: {model}, 基础URL: {base_url}")
    ...
    try:
        ...
        for i, image_path in enumerate(image_files):
            logger.info(f"处理图片 {i + 1}/{len(image_files)}: {image_path.name}")
...
4. 如果上下文与图片内容明显不相关，请忽略上下文，专注于描述图片本身"""
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        logging.info(f"成功生成图片描述: {image_path.name}")
                        success = True
                        break

                    except Exception as e:
                        logging.error(f"处理图片时出错 (尝试 {attempt + 1}/{max_retries}): {str(e)}")
                        if attempt < max_retries - 1:
        ...
        logger.error("导入OpenAI模块失败，无法生成图片描述")
