...
async def load_pdf(
    ...
    # 通过 settings.yaml 文件配置 本地存放 MinerU解析结果文件的路径object has no attribute
    ...
    async def load_file(path: str, group: dict | None) -> pd.DataFrame:
        ...
        try:
            ...
            try:
                # 2. 下载文件
                download_success = await download_output_files(config.mineru_api_url, config.mineru_output_dir,
                                                               config.local_output_dir, doc_id)
                ...
                structured_info = await extract_tables_from_model_json(auto_dir if auto_dir.exists() else doc_local_dir,
                                                                       doc_id)

                # 为表格生成描述
                if structured_info and structured_info.get(
                        "tables") and config.table_description_api_key and config.table_description_model:
                    structured_info = generate_descriptions_for_tables(auto_dir if auto_dir.exists() else doc_local_dir,
                                                                       structured_info, config)

                # 从content_list.json提取图片信息 - 更新路径
                image_info = None
                if content_list_path and content_list_path.exists():
                    image_info = extract_images_from_content_list(auto_dir if auto_dir.exists() else doc_local_dir,
                                                                  doc_id)
                else:
                    log.error(f"content_list.json文件不存在: {content_list_path}")

                # # 为图片生成描述
                if image_info and image_info.get(
                        "images") and config.image_description_api_key and config.image_description_model:
                    image_info = generate_descriptions_for_images(auto_dir if auto_dir.exists() else doc_local_dir,
                                                                  image_info, config)
